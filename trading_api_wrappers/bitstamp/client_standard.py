...
class BitstampStandard(StandardClient):
    ...
    def get_balance(self, currency):
        return float(self.client.account_balance()['%s_balance' % currency])
    ...
    def withdrawal(self, currency, address, amount):
        ...
        if currency.lower() == 'btc':
            method = 'bitcoin'
            version = 1
        elif currency in ["ltc", "eth", "bch"]:
            method = currency
        else:
            raise Exception()
        return self.client.withdrawal(method, address, amount, version=version)
