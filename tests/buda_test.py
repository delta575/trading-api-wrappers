...
class BudaPublicTest(unittest.TestCase):
    ...
    def test_report_candlestick(self):
        ...
        start = end - timedelta(days=30)
        report = self.client.report_candlestick(MARKET_ID, start_at=start, end_at=end)
        for item in report:
...
class BudaAuthTest(unittest.TestCase):
    ...
    def test_quotation(self):
        quotation = self.client.quotation(
            MARKET_ID,
            quotation_type=Buda.QuotationType.ASK_GIVEN_SIZE,
            amount=1,
            limit=1,
        )
    ...
    def test_quotation_market(self):
        quotation = self.client.quotation(
            MARKET_ID, quotation_type=Buda.QuotationType.ASK_GIVEN_SIZE, amount=1
        )
        self.assertIsInstance(quotation, models.Quotation)
    ...
    def test_order_pages(self):
        page, per_page = 2, 10
        order_pages = self.client.order_pages(MARKET_ID, page=page, per_page=per_page)
